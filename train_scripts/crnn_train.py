...
class crnn(train_loop):
    def __init__(self, epoch=0, n_epochs=200, dataset_name="dataset", batch_size=4, npa=1, lr=5e-4, eta_min=1e-6,
                 b1=0.9, b2=0.999, decay_epoch=100, n_cpu=8, hr_height=256, hr_width=256, channels=3,
                 sample_interval=100, checkpoint_interval=100, residual_blocks=23, warmup_batches=500, lambda_adv=5e-3,
                 lambda_pixel=1e-2):
        super().__init__(epoch, n_epochs, dataset_name, batch_size, lr, b1, b2, decay_epoch, n_cpu, hr_height, hr_width,
                         channels, sample_interval, checkpoint_interval, residual_blocks, warmup_batches, lambda_adv,
                         lambda_pixel)
        os.makedirs("ocr_images/training", exist_ok=True)
        os.makedirs("ocr_saved_models", exist_ok=True)
    ...
    def reload(self, b1, b2, batch_size, channels, checkpoint_interval, dataset_name, decay_epoch, epoch, eta_min,
        ...
        # testset = DataLoader(test_dataset, batch_size=batch_size)
        print(count_parameters(self.ocr_model))

    def network_initializers(self):
        ocr_model = fully_conv_model.cnn_attention_ocr(n_layers=8, nclasses=93, model_dim=64, input_dim=3)
        ocr_model = ocr_model.to(self.device).train()
        return ocr_model
    ...
    def optimizers(self, eta_min, lr):
        ...
        ocr_cosine_learning_rate_scheduler = CosineAnnealingLR(optimizer=self.ocr_optimizer, T_max=250000,
                                                                    eta_min=eta_min)
    ...
    def __train(self):
        ...
        for epoch in range(self.opt.epoch, self.opt.n_epochs):
            ...
            for i, ge in enumerate(self.dataloader):
                ...
                plate_encoded = plate_encoded.to(self.device)
    ...
    def __log_progress(self, batches_done, epoch, images, loss, max_elem, max_preds, max_target, n_iter, npa, wer_list):
        ...
        if batches_done % self.opt.checkpoint_interval == 0:
            torch.save(self.ocr_model.state_dict(), "saved_models/ocr_model_%d.pth" % epoch)
        return n_iter
